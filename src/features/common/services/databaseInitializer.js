...
class DatabaseInitializer {
    constructor() {
        ...
        //   macOS: ~/Library/Application Support/Orbit/brdyorbit.db
        //   Windows: %APPDATA%\Orbit\brdyorbit.db
        this.dbPath = path.join(userDataPath, 'brdyorbit.db');
        this.dataDir = userDataPath;

        // The original DB path (read-only location in the package)
        this.sourceDbPath = app.isPackaged
            ? path.join(process.resourcesPath, 'data', 'brdyorbit.db')
            : path.join(app.getAppPath(), 'data', 'brdyorbit.db');
    }
