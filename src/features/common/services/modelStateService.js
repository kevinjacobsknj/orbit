...
class ModelStateService extends EventEmitter {
    ...
    async _initializeFromEnvironment() {
        console.log('[ModelStateService] Initializing providers from environment variables...');
        const config = require('../config/config');
        const aiProviders = config.get('aiProviders');

        if (!aiProviders) {
            console.log('[ModelStateService] No AI providers configuration found in environment');
            return;
        }
        ...
        for (const [envKey, providerId] of Object.entries(envProviderMap)) {
            const providerConfig = aiProviders[envKey];
            if (providerConfig?.apiKey && PROVIDERS[providerId]) {
                try {
                    console.log(`[ModelStateService] Setting up ${providerId} from environment...`);
                    // Skip validation for environment-based keys since they're pre-configured
                    const existingSettings = await providerSettingsRepository.getByProvider(providerId) || {};
                    await providerSettingsRepository.upsert(providerId, { 
                        ...existingSettings, 
                        api_key: providerConfig.apiKey 
                    });
                    console.log(`[ModelStateService] Successfully configured ${providerId} API key`);
                } catch (error) {
                    console.error(`[ModelStateService] Failed to set up ${providerId}:`, error.message);
                }
            }
        }

        // Set up default models from environment if specified
        await this._setDefaultModelsFromEnvironment(aiProviders);
        console.log('[ModelStateService] Environment initialization complete.');
    }

    async _setDefaultModelsFromEnvironment(aiProviders) {
        console.log('[ModelStateService] Setting default models from environment...');
        
        // Get current selected models
        const currentModels = await this.getSelectedModels();
        
        // Only set defaults if no models are currently selected
        if (!currentModels.llm && aiProviders.openai?.model) {
            const openaiModel = aiProviders.openai.model;
            console.log(`[ModelStateService] Setting default LLM model: ${openaiModel}`);
            await this.setSelectedModel('llm', openaiModel);
        }

        if (!currentModels.llm && aiProviders.anthropic?.model) {
            const anthropicModel = aiProviders.anthropic.model;
            console.log(`[ModelStateService] Setting default LLM model: ${anthropicModel}`);
            await this.setSelectedModel('llm', anthropicModel);
        }

        // For STT, prefer Deepgram if available, otherwise OpenAI
        if (!currentModels.stt) {
            if (aiProviders.deepgram?.apiKey) {
                console.log('[ModelStateService] Setting default STT model: deepgram-general');
                await this.setSelectedModel('stt', 'deepgram-general');
            } else if (aiProviders.openai?.apiKey) {
                console.log('[ModelStateService] Setting default STT model: whisper-1');
                await this.setSelectedModel('stt', 'whisper-1');
            }
        }
    }
