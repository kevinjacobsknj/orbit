...
function mapModelName(model) {
    const modelMappings = {
        ...
        'gemini-ultra-2.0': 'gemini-1.5-flash'
    };
...
async function createSTT({ apiKey, language = "en-US", callbacks = {}, ...config }) {
  ...
  const session = await liveClient.live.connect({
    ...
    model: mapModelName(process.env.GOOGLE_MODEL || 'gemini-2.0-flash-exp'),
    callbacks: {
      ...callbacks,
      onMessage: (msg) => {
 ...
 */
function createLLM({ apiKey, model = process.env.GOOGLE_MODEL || "gemini-2.0-flash-exp", temperature = 0.7, maxTokens = 8192, ...config }) {
 ...
 */
function createStreamingLLM({ apiKey, model = process.env.GOOGLE_MODEL || "gemini-2.0-flash-exp", temperature = 0.7, maxTokens = 8192, ...config }) {
  const client = new GoogleGenerativeAI(apiKey)
  const actualModel = mapModelName(model)
