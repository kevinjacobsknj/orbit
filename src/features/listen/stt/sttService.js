...
class SttService {
    ...
    sendToRenderer(channel, data) {
        ...
        // Send to Listen window for normal operation
        const listenWindow = windowPool?.get('listen');
        if (listenWindow && !listenWindow.isDestroyed()) {
            listenWindow.webContents.send(channel, data);
        }
        
        // Also send STT events to Ask window for voice input functionality
        if (channel === 'stt-update') {
            const askWindow = windowPool?.get('ask');
            if (askWindow && !askWindow.isDestroyed()) {
                console.log('[SttService] Sending STT event to Ask window:', {
                ...
                console.log('[SttService] Ask window not available for STT event');
            }
        }
    }
