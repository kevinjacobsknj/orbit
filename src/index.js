...
function setupProtocolHandling() {
    ...
    try {
        if (!app.isDefaultProtocolClient('brdyorbit')) {
            const success = app.setAsDefaultProtocolClient('brdyorbit');
            if (success) {
                console.log('[Protocol] Successfully set as default protocol client for brdyorbit://');
            } else {
                console.warn('[Protocol] Failed to set as default protocol client - this may affect deep linking');
            }
        } else {
            console.log('[Protocol] Already registered as default protocol client for brdyorbit://');
        }
    } catch (error) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        for (const arg of commandLine) {
            if (arg && typeof arg === 'string' && arg.startsWith('brdyorbit://')) {
    ...
    app.on('open-url', (event, url) => {
        ...
        if (!url || !url.startsWith('brdyorbit://')) {
...
async function handleCustomUrl(url) {
    try {
        ...
        if (!url || typeof url !== 'string' || !url.startsWith('brdyorbit://')) {
...
async function startWebStack() {
  ...
  process.env.brdyorbit_API_PORT = apiPort.toString();
  process.env.brdyorbit_API_URL = `http://localhost:${apiPort}`;
  process.env.brdyorbit_WEB_PORT = frontendPort.toString();
  process.env.brdyorbit_WEB_URL = `http://localhost:${frontendPort}`;

  console.log(`🌍 Environment variables set:`, {
    brdyorbit_API_URL: process.env.brdyorbit_API_URL,
    brdyorbit_WEB_URL: process.env.brdyorbit_WEB_URL
  });

  const createBackendApp = require('../brdyorbit_web/backend_node');
  const nodeApi = createBackendApp(eventBridge);

  const staticDir = app.isPackaged
    ? path.join(process.resourcesPath, 'out')
    : path.join(__dirname, '..', 'brdyorbit_web', 'out');
  ...
  if (!fs.existsSync(staticDir)) {
    ...
    console.error(`Please run 'npm run build' inside the 'brdyorbit_web' directory first.`);
    console.error(`============================================================`);
    app.quit();
    return;
  }
