...
contextBridge.exposeInMainWorld('api', {
  ...
  askView: {
    ...
    getIsDragging: () => ipcRenderer.invoke('get-is-dragging'),
    
    // Message Handling
    sendMessage: (text) => ipcRenderer.invoke('ask:sendQuestionFromAsk', text),
    
    // STT Operations for Ask window voice input
    initializeSTT: (language) => ipcRenderer.invoke('stt:initializeSessions', language),
    closeSTT: () => ipcRenderer.invoke('stt:closeSessions'),
    sendMicAudio: (data) => ipcRenderer.invoke('stt:sendMicAudio', data),
  ...
  sttView: {
    ...
    onSttUpdate: (callback) => {
      const wrappedCallback = (event, data) => {
        console.log('[Preload] STT event received, calling Ask window callback:', {
          hasEvent: !!event,
          hasData: !!data,
        ...
        try {
          // Call callback with proper arguments format
          callback.apply(null, [event, data]);
        } catch (error) {
          console.error('[Preload] Error calling STT callback:', error);
        }
      };
      console.log('[Preload] Setting up STT listener with wrapped callback');
      ipcRenderer.on('stt-update', wrappedCallback);
    },
