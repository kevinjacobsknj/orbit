...
module.exports = {
  ...
  initialize() {
    ...
    ipcMain.handle('get-web-url', () => process.env.brdyorbit_WEB_URL || 'http://localhost:3000');
    ...
    // Direct STT operations for Ask window
    ipcMain.handle('stt:initializeSessions', async (event, language = 'en') => {
      try {
        console.log('[FeatureBridge] Initializing STT sessions for Ask window');
        await listenService.sttService.initializeSttSessions(language);
        return { success: true };
      } catch (error) {
        console.error('[FeatureBridge] STT initialization failed:', error.message);
        return { success: false, error: error.message };
      }
    });
    ipcMain.handle('stt:closeSessions', async () => {
      try {
        console.log('[FeatureBridge] Closing STT sessions for Ask window');
        await listenService.sttService.closeSessions();
        return { success: true };
      } catch (error) {
        console.error('[FeatureBridge] STT close failed:', error.message);
        return { success: false, error: error.message };
      }
    });
    ipcMain.handle('stt:sendMicAudio', async (event, { data, mimeType }) => {
      try {
        await listenService.sttService.sendMicAudioContent(data, mimeType);
        return { success: true };
      } catch (error) {
        console.error('[FeatureBridge] STT sendMicAudio failed:', error.message);
        return { success: false, error: error.message };
      }
    });
