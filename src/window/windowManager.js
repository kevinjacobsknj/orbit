...
// Add debounce variables for drag operations
let dragDebounceTimer = null;
let isDragging = false;
let stableHeaderBounds = null;



function updateChildWindowLayouts(animated = true) {
    // Only skip if actively animating (not dragging)
    if (movementManager.isAnimating && !isDragging) {
        console.log(`[WindowManager] BLOCKED - isAnimating: ${movementManager.isAnimating}, isDragging: ${isDragging}`);
        return;
    }
    ...
    // Get current header position to verify it's using the right coordinates
    const header = windowPool.get('header');
    let headerBounds = null;
    
    if (isDragging && stableHeaderBounds) {
        // During drag, use stable bounds but update only position
        const currentBounds = header ? header.getBounds() : null;
        if (currentBounds) {
            headerBounds = {
                ...stableHeaderBounds,
                x: currentBounds.x,
            ...
            console.log(`[WindowManager] Using stable header bounds during drag: x=${headerBounds.x}, y=${headerBounds.y}, w=${headerBounds.width}, h=${headerBounds.height}`);
        }
    } else {
        headerBounds = header ? header.getBounds() : null;
        if (headerBounds) {
            console.log(`[WindowManager] Header at: x=${headerBounds.x}, y=${headerBounds.y}, w=${headerBounds.width}, h=${headerBounds.height}, isDragging=${isDragging}`);
        }
    }

    const newLayout = layoutManager.calculateFeatureWindowLayout(visibleWindows, headerBounds, isDragging);
    
    // Log the calculated layout
    if (newLayout.ask) {
        console.log(`[WindowManager] Ask window layout: x=${newLayout.ask.x}, y=${newLayout.ask.y}, w=${newLayout.ask.width}, h=${newLayout.ask.height}`);
    }
    
    // During drag, move windows immediately without animation to follow the header
    if (isDragging) {
        console.log(`[WindowManager] Moving windows during drag`);
        movementManager.animateLayout(newLayout, false);
    } else {
...
const adjustWindowHeight = (winName, targetHeight) => {
    // Don't adjust window height if we're currently dragging
    if (isDragging) {
        console.log(`[WindowManager] Blocked adjustWindowHeight(${winName}, ${targetHeight}) during drag`);
        return;
    }
...
function setupWindowController(windowPool, layoutManager, movementManager) {
    ...
    internalBridge.on('window:moveStep', ({ direction }) => {
        ...
        if (header) { 
            ...
            const newChildLayout = layoutManager.calculateFeatureWindowLayout(visibleWindows, futureHeaderBounds, isDragging);
...
async function handleWindowVisibilityRequest(windowPool, layoutManager, movementManager, name, shouldBeVisible) {
    ...
    if (name === 'listen' || name === 'ask') {
        ...
        const targetLayout = layoutManager.calculateFeatureWindowLayout(finalVisibility, null, isDragging);
...
function createWindows() {
    ...
    // Update layout immediately during drag operations so windows follow the header
    header.on('moved', () => {
        ...
        // Set the dragging flag when movement starts
        if (!isDragging) {
            console.log('[WindowManager] DRAG STARTED');
            // Capture stable header bounds at the start of drag
            stableHeaderBounds = header.getBounds();
            console.log(`[WindowManager] Captured stable bounds: w=${stableHeaderBounds.width}, h=${stableHeaderBounds.height}`);
        }
        isDragging = true;

        // Update layout immediately during drag so windows follow the header
        updateChildWindowLayouts(false);

        // Reset dragging flag after movement stops
        dragDebounceTimer = setTimeout(() => {
            console.log('[WindowManager] DRAG ENDED');
            isDragging = false;
            stableHeaderBounds = null;
            dragDebounceTimer = null;
        }, 200);
    });
    ...
    // Removed problematic resize event handler that was causing infinite loop
    // header.on('resize', () => {
    //     if (!isDragging) {
    //         updateChildWindowLayouts(false);
    //     }
