...
class WindowLayoutManager {
    ...
    calculateFeatureWindowLayout(visibility, headerBoundsOverride = null, isDragging = false) {
        const header = this.windowPool.get('header');
        const headerBounds = headerBoundsOverride || (header ? header.getBounds() : null);

        if (!headerBounds) return {};
        
        // Allow layout calculations during drag - we need windows to follow the header
        // The resize event handler removal prevents the infinite loop
        ...
        // Debug logging disabled to prevent potential getBounds() side effects
        // if (askVis) {
