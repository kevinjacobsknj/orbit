...
export class AskView extends LitElement {
    ...
    connectedCallback() {
        ...
        // Track window focus to prevent resizing during drag
        window.addEventListener('blur', this.handleWindowBlur);
        window.addEventListener('focus', this.handleWindowFocus);
        
        // Disable auto-resizing for the first few seconds to prevent expansion during initial positioning
        ...
        this.resizeObserver = new ResizeObserver(entries => {
            // Skip resize observer entirely during drag operations or initialization
            if (this.isDragging || this.isInitializing) {
                return;
            }
    ...
    disconnectedCallback() {
        ...
        // Mouse move listener was inline, no need to remove
    ...
    async startRecording() {
        try {
            ...
            // The actual STT integration would be implemented here
    ...
    handleWindowBlur() {
        ...
        // Keep dragging state longer to be safe
        setTimeout(() => {
            this.isDragging = false;
        }, 500);
    }
    ...
    handleWindowFocus() {
        // Delay resetting isDragging to ensure drag operation is complete
        setTimeout(() => {
            this.isDragging = false;
        }, 500);
    }
    ...
    updated(changedProperties) {
        ...
        // Only adjust window height for content changes, not frequent UI updates
        if ((changedProperties.has('currentResponse') && this.currentResponse) || 
            (changedProperties.has('showTextInput') && !this.isDragging) ||
            (changedProperties.has('isLoading') && !this.isDragging)) {
    ...
    render() {
        ...
        return html`
            <div class="ask-container">
                ...
                <!-- Suggestions Container -->
                <div class="suggestions-container ${!this.showTextInput ? 'hidden' : ''}">
    ...
    adjustWindowHeight() {
        if (!window.api || this.isDragging) return;
        ...
        this.updateComplete.then(() => {
            ...
            // Only resize if content height has actually changed significantly
            if (Math.abs(idealHeight - this.lastContentHeight) < 10) {
                return;
            }

            this.lastContentHeight = idealHeight;
    ...
    // Throttled wrapper to avoid excessive IPC spam (executes at most once per 200ms)
    adjustWindowHeightThrottled() {
        if (this.isThrottled || this.isDragging) return;

        this.isThrottled = true;
        setTimeout(() => {
            this.adjustWindowHeight();
            this.isThrottled = false;
        }, 200); // Increased delay to 200ms to reduce frequency
    }
}
